...
public class RankedKnowledge_CommandLineArgs extends AbstractArgs {
	// ************************************************************
	/** データセット名 */
	...
	protected void load(String[] args) {
		...
		if(args.length < n) {
			...
			System.out.println("---");
			System.out.print(RankedKnowledge_CommandLineArgs.getParamsString());
			System.out.println("---");
			return;
		}
		...
		experimentID = args[2];
		RankedKnowledge_Consts.LEARNING_EXPERIMENT_ID_DIR = Consts.ROOTFOLDER + File.separator + "MixedKB" + File.separator + dataName + File.separator + experimentID;
		...
		testFile = args[5];
		if(args.length > 5) {
			RankedKnowledge_Consts.FUZZY_TERMS_NUM = Integer.valueOf(args[6]);
		}
