		...
		try {
			if(solutionList.size() > 0) {
				for(int i = 0; i < solutionList.size(); i++) {
					...
					PittsburghSolution_Basic solution = (PittsburghSolution_Basic) solutionList.get(i);
	...
	public Element printSolutionsToElement(List<? extends Solution<?>> solutionList) {
		...
		if(solutionList.size() > 0) {
			for(int i = 0; i < solutionList.size(); i++) {
				...
				//if-then rule一覧
				Classifier classifier = (Classifier) solution.getClassifier();
				XML_manager.addElement(individual, classifier.toElemnt());
